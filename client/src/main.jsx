import React,{useEffect,useState} from 'react';import{createRoot}from'react-dom/client';import{LayoutDashboard,ClipboardList,Megaphone,Plus,LogOut,ShieldCheck,Clock,CheckCircle2,AlertTriangle,Camera,Menu,X}from'lucide-react';import'./style.css';
const API=import.meta.env.VITE_API_URL||'http://localhost:5000/api';const token=()=>localStorage.getItem('token');const user=()=>JSON.parse(localStorage.getItem('user')||'null');
async function api(path,opt={}){const h=opt.body instanceof FormData?{'Authorization':`Bearer ${token()}`}:{'Content-Type':'application/json','Authorization':`Bearer ${token()}`};const r=await fetch(API+path,{...opt,headers:{...h,...(opt.headers||{})}});const d=await r.json();if(!r.ok)throw Error(d.error||'Request failed');return d}
function Login({onLogin}){const[register,setRegister]=useState(false),[f,setF]=useState({name:'',email:'',password:''}),[err,setErr]=useState('');async function submit(e){e.preventDefault();try{const d=await api(register?'/auth/register':'/auth/login',{method:'POST',body:JSON.stringify(f)});localStorage.setItem('token',d.token);localStorage.setItem('user',JSON.stringify(d.user));onLogin(d.user)}catch(e){setErr(e.message)}}return <div className="auth"><div className="auth-card"><div className="brand"><ShieldCheck size={34}/><div><b>SocietyCare</b><span>Maintenance Portal</span></div></div><h1>{register?'Create resident account':'Welcome back'}</h1><p className="muted">{register?'Raise and track your society complaints.':'Sign in to manage your maintenance requests.'}</p><form onSubmit={submit}>{register&&<input placeholder="Full name" value={f.name} onChange={e=>setF({...f,name:e.target.value})}/>}<input type="email" placeholder="Email" value={f.email} onChange={e=>setF({...f,email:e.target.value})}/><input type="password" placeholder="Password" value={f.password} onChange={e=>setF({...f,password:e.target.value})}/>{err&&<div className="error">{err}</div>}<button className="primary">{register?'Register':'Sign in'}</button></form><button className="link" onClick={()=>setRegister(!register)}>{register?'Already have an account? Sign in':'New resident? Create account'}</button>{!register&&<div className="demo"><b>Admin demo</b><br/>admin@society.local / Admin@123</div>}</div></div>}
function App(){const[u,setU]=useState(user());if(!u)return <Login onLogin={setU}/>;return <Shell u={u} onLogout={()=>{localStorage.clear();setU(null)}}/>}
function Shell({u,onLogout}){const[tab,setTab]=useState('dashboard');const[open,setOpen]=useState(false);return <div className="app"><aside className={open?'open':''}><div className="side-brand"><ShieldCheck/><span>SocietyCare</span></div><nav>{u.role==='admin'&&<button className={tab==='dashboard'?'active':''} onClick={()=>{setTab('dashboard');setOpen(false)}}><LayoutDashboard/>Dashboard</button>}<button className={tab==='complaints'?'active':''} onClick={()=>{setTab('complaints');setOpen(false)}}><ClipboardList/>Complaints</button><button className={tab==='notices'?'active':''} onClick={()=>{setTab('notices');setOpen(false)}}><Megaphone/>Notice Board</button></nav><div className="side-bottom"><div className="user"><div className="avatar">{u.name[0]}</div><div><b>{u.name}</b><span>{u.role}</span></div></div><button onClick={onLogout}><LogOut/>Logout</button></div></aside><main><header><button className="menu" onClick={()=>setOpen(!open)}>{open?<X/>:<Menu/>}</button><div><h2>{tab==='dashboard'?'Dashboard':tab==='complaints'?'Complaints':'Notice Board'}</h2><span className="muted">{u.role==='admin'?'Administration workspace':'Resident workspace'}</span></div></header>{tab==='dashboard'&&u.role==='admin'?<Dashboard/>:tab==='complaints'?<Complaints admin={u.role==='admin'}/>:<Notices admin={u.role==='admin'}/>}</main></div>}
